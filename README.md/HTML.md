...
- 행과 열의 조합으로 웹 페이지의 레이아웃을 구성하기 위한 속성
- 각의 셀은 컨텐츠를 배치하는 데 사용
- 그리드 아이템을 자유롭게 배치할 수 있음
- 그리드 아이템의 크기와 위치를 조절하여 유연한 레이아웃을 만들 수 있음/그리드는 반응형 디자인에 사용 가능
- 부모 요소 : Grid Container(그리드 컨테이너)
- 자식 요소 : Grid Item(그리드 아이템) 
- 아이템 한 칸의 영역을 그리드 셀 이라고 함
...
- display
  ...
  - **fr** : 그리드에서 열의 크기를 지정하는 데 사용되는 상대적인 단위/사용 가능한 공간을 원하는 비율로 분할할 수 있음
